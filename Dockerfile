...
# builds importvet
FROM base AS importvet-build
WORKDIR /src/cmd/importvet
RUN --mount=type=cache,target=/root/.cache/go-build go build -ldflags "-s -w" -o /importvet

# runs gofumpt
FROM base AS lint-gofumpt
RUN find . -name '*.pb.go' | xargs -r rm
RUN FILES="$(gofumports -l -local github.com/talos-systems/importvet .)" && test -z "${FILES}" || (echo -e "Source code is not formatted with 'gofumports -w -local github.com/talos-systems/importvet .':\n${FILES}"; exit 1)
...
FROM scratch AS importvet
COPY --from=importvet-build /importvet /importvet

FROM scratch AS unit-tests
COPY --from=unit-tests-run /src/coverage.txt /coverage.txt

FROM importvet AS image-importvet
COPY --from=image-fhs / /
